# -*- coding: utf-8 -*-
"""
手动插入 Agnes AI 热门文章到 TechNews 数据库
运行一次即可，后续由爬虫自动维护
"""
import sys
import os
...
def main():
    print(f'准备插入 {len(AGNES_ARTICLES)} 篇 Agnes AI 文章...\n')
    success = 0
    skipped = 0
    ...
    for i, article in enumerate(AGNES_ARTICLES, 1):
        ...
        if result is not None:
            success += 1
            print(f'  [+] [{i}/{len(AGNES_ARTICLES)}] {article["title"][:55]}')
        else:
            skipped += 1
            print(f'  [-] [{i}/{len(AGNES_ARTICLES)}] 已存在，跳过：{article["title"][:55]}')

    print(f'\n完成！新增 {success} 篇，跳过 {skipped} 篇（已存在）')

    # 验证
    conn = get_db()
    total = conn.execute(
        "SELECT COUNT(*) FROM articles WHERE keywords LIKE '%Agnes%' OR title LIKE '%Agnes%'"
    ).fetchone()[0]
    conn.close()
    print(f'数据库中 Agnes 相关文章总数：{total} 篇')
